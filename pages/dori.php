...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($post_action === 'add' || $post_action === 'edit') {
        ...
        try {
            if ($post_action === 'add') {
                $stmt = $db->prepare("INSERT INTO dori_calculos (projeto_id, equipamento_id, nome_zona, objetivo, nivel_risco, largura_cena_m, distancia_camera_m, resolucao_horizontal, sensor_largura_mm) VALUES (?,?,?,?,?,?,?,?,?)");
                $stmt->execute([$projeto_id, $equipamento_id, $nome_zona, $objetivo, $nivel_risco, $largura_cena, $distancia_camera, $resolucao, $sensor]);
                $_SESSION['flash'] = 'Cálculo DORI adicionado.';
            } else {
                $stmt = $db->prepare("UPDATE dori_calculos SET projeto_id=?, equipamento_id=?, nome_zona=?, objetivo=?, nivel_risco=?, largura_cena_m=?, distancia_camera_m=?, resolucao_horizontal=?, sensor_largura_mm=? WHERE id=?");
                $stmt->execute([$projeto_id, $equipamento_id, $nome_zona, $objetivo, $nivel_risco, $largura_cena, $distancia_camera, $resolucao, $sensor, $id]);
...
?>
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:20px">
        ...
        <div class="card" id="preview-card">
            ...
            <div style="font-size:0.9rem">
                <?php
                    $tmp_ppm = $d['resolucao_horizontal'] / max($d['largura_cena_m'], 0.5);
                    $tmp_nec = $preview_objetivos[$d['objetivo']] ?? 125;
                    $tmp_focal = ($d['sensor_largura_mm'] * $d['distancia_camera_m']) / max($d['largura_cena_m'], 0.5);
                    $tmp_fov = 2 * rad2deg(atan2($d['sensor_largura_mm'], 2 * max($d['distancia_camera_m'], 0.5)));
                ?>
                ...
                <div class="info-row"><span class="label">Distância Focal Recomendada</span><span class="value"><?= fmtNumero($tmp_focal,1) ?> mm</span></div>
                <div class="info-row"><span class="label">FOV Horizontal</span><span class="value"><?= fmtNumero($tmp_fov,1) ?>°</span></div>
    ...
    function updatePreview() {
        ...
        const ppm = resolucao / largura;
        const focal = (sensor * distancia) / largura;
        const fov = 2 * Math.atan2(sensor, 2 * distancia) * 180 / Math.PI;
        ...
        const previewHtml = `
            ...
            <div class="info-row"><span class="label">Dist. Focal Recomendada</span><span class="value">${focal.toFixed(1)} mm</span></div>
            <div class="info-row"><span class="label">FOV Horizontal</span><span class="value">${fov.toFixed(1)}°</span></div>
            <div class="info-row"><span class="label">Nível Alcançado</span><span class="value">${
                ppm >= 250 ? '<span style="color:#EF5350">I — Identificação</span>' :
                ppm >= 125 ? '<span style="color:#FFB74D">R — Reconhecimento</span>' :
                ppm >= 62 ? '<span style="color:#AED581">O — Observação</span>' :
                ppm >= 25 ? '<span style="color:#90CAF9">D — Detecção</span>' :
                '<span style="color:#EF5350">Abaixo do mínimo</span>'
            }</span></div>
        `;
